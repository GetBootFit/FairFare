...
export async function POST(req: NextRequest) {
  ...
  let tokenPayload: Awaited<ReturnType<typeof verifyTokenFromRequest>> | null =
    null;
  if (process.env.NEXT_PUBLIC_PAYWALL_ENABLED !== "false") {
    ...
    if (
      ...
      return Response.json(
        { error: "Token not valid for taxi" },
        { status: 403 },
      );
    }
  }
  ...
  try {
    ...
    const rateData =
      route.cityCandidates
        .map((c) => findCityRate(`${c} ${route.country}`))
        .find(Boolean) ?? null;
    if (!rateData) {
    ...
    const aiInfo = await getTaxiAiInfo(
      route.city,
      route.country,
      locale,
      fareRange.note ?? undefined,
    );
    ...
    const message =
      err instanceof Error ? err.message : "Failed to fetch taxi data";
