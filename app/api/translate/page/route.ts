...
function getAboutContent() {
  ...
  return {
    ...
    step3_desc: 'Pay once with card, Apple Pay, or Google Pay. No account, no subscription. Choose a single query, DayPass, or 20-check bundle.',
...
function getFaqContent() {
  ...
  return {
    ...
    a1_b2: `DayPass — ${pass}: unlimited checks for one country, 24 hrs from first use`,
    ...
    a7: `It depends on the option chosen. A single query (${single}) covers one result. A DayPass (${pass}) gives you unlimited taxi and tipping checks for one country for 24 hrs from first use. A 20-query bundle (${bundle}) gives you 20 queries stored on your device, valid for 90 days across any city or country.`,
