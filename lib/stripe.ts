...
export async function createCheckoutSession(params: CheckoutParams): Promise<string> {
  ...
  const productLabels: Record<string, { name: string; description: string }> = {
    ...
    country_pass_taxi: {
      name: `Hootling — ${country ? country.charAt(0).toUpperCase() + country.slice(1) : ''} Trip Pass`,
      description: 'Unlimited taxi & tipping checks for this country · valid 24 hours · instant access',
    },
