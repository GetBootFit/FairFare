...
const ALIASES: Record<string, string> = {
  ...
  // Melbourne — airport/LGA variants + inner-city suburbs Google Places returns
  ...
  // Inner suburbs — Google often returns these instead of "Melbourne"
  southbank: "melbourne",
  south_yarra: "melbourne",
  st_kilda: "melbourne",
  docklands: "melbourne",
  carlton: "melbourne",
  fitzroy: "melbourne",
 ...
 */
export function findCityRate(
  cityString: string,
): (CityRate & { city: string }) | null {
  ...
  if (firstToken && firstToken.length >= 4) {
    const match = sortedKeys.find(
      (k) => k.startsWith(firstToken) || firstToken.startsWith(k),
    );
