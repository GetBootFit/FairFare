...
export const AIRPORT_DATA: Record<string, AirportData> = {
  ...
  MEL: {
    ...
    minimumFare: 4.20,
    taxiColor: 'yellow (Silver Service black available)',
    approxCityFare: 'A$55–80 to Melbourne CBD',
    routes: [
      { label: 'Melbourne CBD / Flinders Street', km: 23, note: 'Tullamarine Freeway toll A$4.49 + possible CityLink toll' },
      { label: 'Southbank / South Yarra', km: 26, note: 'CityLink toll ~A$5–8 extra' },
      { label: 'St Kilda', km: 30 },
      { label: 'Docklands / Etihad Stadium', km: 22 },
      { label: 'Brighton / Elwood', km: 38 },
      { label: 'Geelong city centre', km: 75, note: 'negotiate a fixed price; ~A$130–160' },
    ],
    tips: [
      'All metered taxis charge a standard flag fall of A$4.20 plus A$2.19/km — always use the meter.',
      'CityLink and Tullamarine Freeway tolls are extra and paid by the passenger; routes through the city incur extra tolls.',
      'Silver Service (black luxury) taxis cost more but offer guaranteed booking and a cleaner vehicle.',
      '13Cabs and Ingogo apps allow pre-booking with an upfront estimate.',
      'The SkyBus operates to the CBD every 10–30 minutes for A$22 — great for solo travellers with one bag.',
    ],
    scams: [
      'Driver avoids CityLink (toll road) to reduce cost to themselves, extending your travel time significantly.',
      'Claims of "credit card surcharge" exceeding the permitted 5% maximum.',
      'Unofficial vehicles outside the taxi rank — always board at the official rank on the ground level.',
    ],
    alternatives: [
      'SkyBus — A$22 to CBD (Southern Cross Station), runs every 10–30 min',
      'Uber / DiDi / Ola — price shown upfront, often 10–20% cheaper than taxis',
      'Route 901 SmartBus — A$5.70 to Broadmeadows & Sunshine (slow but cheap)',
    ],
  },
  ...
  IST: {
    ...
    ratePerKm: 45,
    minimumFare: 80,
    taxiColor: 'yellow (official Istanbul taxi)',
    approxCityFare: '₺800–1,500 to city centre (Taksim/Sultanahmet)',
    routes: [
      { label: 'Taksim / Beyoğlu', km: 40, note: 'highway toll ~₺30–60 extra; allow 45–70 min' },
      { label: 'Sultanahmet / Historic Peninsula', km: 42, note: 'highway toll + bridge/tunnel toll extra' },
      { label: 'Levent / Maslak (business district)', km: 35 },
      { label: 'Kadıköy (Asian side)', km: 55, note: 'Eurasia Tunnel toll ~₺120 extra' },
      { label: 'Şişli / Nişantaşı', km: 38 },
    ],
    tips: [
      'All official Istanbul taxis are metered and yellow — insist the meter is turned on at the start of the trip.',
      'Highway and bridge/tunnel tolls are paid by the passenger on top of the metered fare.',
      'The Eurasia Tunnel (Avrasyatüneli) is the fastest route to the Asian side but adds a substantial toll.',
      'BiTaksi and Uber both operate in Istanbul — fixed price shown upfront, recommended over street taxis.',
      'Allow extra time: Istanbul Airport is 35–45 km from the city centre and traffic can be severe.',
    ],
    scams: [
      'Driver takes the longer D-100 highway route instead of the faster toll road — confirm the route upfront.',
      'Inflated "airport fee" added on arrival — only the metered fare plus actual tolls apply.',
      'Unlicensed private drivers ("korsan taksi") approach in arrivals — only use yellow metered taxis from the official rank.',
      'Metre started at the night tariff (tariff 2) during daytime — check the tariff indicator on the meter.',
    ],
    alternatives: [
      'Havaist airport bus — ₺120–200 to major Istanbul hubs (Taksim, Şişhane, Yenikapı)',
      'BiTaksi / Uber — fixed upfront price, widely used in Istanbul',
      'Istanbul Airport Metro (M11) — connects to Gayrettepe then metro network; ~50 min to city',
    ],
  },
  ...
  KUL: {
    ...
    ratePerKm: 1.25,
    minimumFare: 3.00,
    taxiColor: 'red (budget metered) or white (KLIA Premier fixed-rate)',
    approxCityFare: 'RM80–120 to KL city centre (KLIA Premier fixed-rate recommended)',
    routes: [
      { label: 'KLCC / Bukit Bintang (city centre)', km: 58, note: 'KLIA Premier fixed rate RM80–100; metered ~RM70–100 + tolls' },
      { label: 'Chow Kit / Brickfields', km: 56 },
      { label: 'Bangsar / Mid Valley', km: 55 },
      { label: 'Petaling Jaya / Subang', km: 45 },
      { label: 'Putrajaya / Cyberjaya', km: 20, note: 'government district; RM30–50' },
    ],
    tips: [
      'KLIA Premier Taxis (white) offer fixed-rate counters inside the terminal — buy a coupon before exiting to avoid meter disputes.',
      'Budget taxis (red) are metered and cheaper but less predictable; ensure the meter is running.',
      'Grab is extremely reliable from KLIA and offers transparent pricing — usually 20–30% cheaper than taxis.',
      'KLIA Ekspres train runs to KL Sentral in 28 minutes for RM55 — fastest and most comfortable option.',
      'Expressway tolls are included in the KLIA Premier fixed-rate coupons.',
    ],
    scams: [
      'Driver offers a "cheaper" metered fare but bypasses the toll roads, doubling journey time.',
      'Tout inside the terminal approaches with unlicensed fixed-rate offers above the coupon price.',
      'Meter starts before leaving the terminal car park — the meter should begin when you exit the airport grounds.',
    ],
    alternatives: [
      'KLIA Ekspres — RM55 to KL Sentral, 28 min (fastest option)',
      'Grab — transparent pricing, very reliable from KLIA',
      'KLIA Transit — RM55 with stops at Putrajaya/Cyberjaya and Salak Tinggi',
    ],
  },
  ...
  BCN: {
    ...
    ratePerKm: 1.13,
    minimumFare: 2.30,
    taxiColor: 'black and yellow (Taxi Barcelona)',
    approxCityFare: '€39 flat rate to city centre',
    blogSlug: 'how-much-does-a-taxi-cost-in-barcelona',
    routes: [
      { label: 'Barcelona city centre / Eixample', km: 15, note: 'FLAT RATE €39 (day) / €43 (night & weekends)' },
      { label: 'Las Ramblas / Gothic Quarter', km: 14, note: 'within flat-rate zone' },
      { label: 'Barceloneta / Waterfront', km: 17, note: 'flat-rate zone' },
      { label: 'Gràcia / Camp Nou', km: 16, note: 'flat-rate zone' },
      { label: 'Sitges', km: 42, note: 'outside flat-rate zone — metered, ~€65–85' },
    ],
    tips: [
      'A regulated flat rate of €39 (day) / €43 (night and weekends) applies between BCN airport and the Barcelona city centre.',
      'The flat rate covers the entire city within the municipal boundary — no extras for different districts.',
      'Supplement of €1 per suitcase and €3.10 airport supplement are included in the flat rate.',
      'Uber and Cabify operate from BCN — often comparable in price to the flat rate.',
      'The L9 Sud Metro (Airport line) runs to the city for €5.15 but requires a change to reach most hotels — allow 45–60 min.',
    ],
    scams: [
      'Driver uses the meter instead of the flat rate — the flat rate always applies to the city centre.',
      'Driver claims the flat rate "doesn\'t cover" your specific hotel — it covers all Barcelona city addresses.',
      'Unlicensed taxis outside the terminal with inflated fixed prices — always use the official rank.',
      'Extra luggage charges beyond the permitted €1/suitcase rate.',
    ],
    alternatives: [
      'L9 Sud Metro — €5.15 to Zona Universitària (change for city centre), 35 min',
      'Aerobus — €6.75 to Plaça Catalunya, 35 min, runs every 5 min',
      'Uber / Cabify — competitive with flat rate, price shown upfront',
    ],
  },
  ...
  MUC: {
    ...
    ratePerKm: 2.20,
    minimumFare: 4.80,
    taxiColor: 'cream/beige (Munich standard)',
    approxCityFare: '€70–95 to Munich city centre',
    routes: [
      { label: 'Munich city centre / Marienplatz', km: 35, note: 'A9 motorway; 40–55 min' },
      { label: 'Schwabing / Maxvorstadt', km: 32 },
      { label: 'Oktoberfest grounds / Theresienwiese', km: 37 },
      { label: 'Bogenhausen / Englischer Garten', km: 30 },
      { label: 'Augsburg city centre', km: 70, note: '~€140–170; S-Bahn + ICE is much faster' },
    ],
    tips: [
      'All Munich taxis are metered — the meter starts at €4.80 with a rate of €2.20/km.',
      'S-Bahn S1 and S8 lines both run to Munich Hbf in 40 minutes for €13.20 — significantly cheaper.',
      'During Oktoberfest (late September/early October) taxi demand surges — book in advance or allow long wait times.',
      'Tipping 5–10% is customary; rounding up to the nearest euro is common.',
      'Taxis accept credit cards; always confirm before departure.',
    ],
    scams: [
      'Driver takes the slower surface route via the B471 instead of the A9 motorway — the motorway is faster and usually cheaper.',
      'Unlicensed private vehicles outside the terminal — only use cream-coloured taxis from the official rank.',
      'Inflated fixed-rate offers during Messe trade fair weeks — always insist on the meter.',
    ],
    alternatives: [
      'S-Bahn S1/S8 — €13.20 to Munich Hbf, 40 min',
      'Lufthansa Express Bus — €13.50 to Munich Hbf, 45 min',
      'Uber — available in Munich, price shown upfront',
    ],
  },
  ...
  HKG: {
    ...
    ratePerKm: 12.00,
    minimumFare: 27.00,
    taxiColor: 'red (urban/Kowloon & HK Island)',
    approxCityFare: 'HK$350–550 to Kowloon / HK$400–650 to HK Island',
    routes: [
      { label: 'Tsim Sha Tsui / Kowloon', km: 35, note: 'Western Harbour Crossing toll ~HK$80 extra' },
      { label: 'Mong Kok / Yau Ma Tei', km: 33, note: 'Western Harbour Crossing toll ~HK$80 extra' },
      { label: 'Central / Admiralty (HK Island)', km: 40, note: 'tunnel toll ~HK$55–80 extra' },
      { label: 'Wan Chai / Causeway Bay', km: 42, note: 'tunnel toll ~HK$55–80 extra' },
      { label: 'Tung Chung (adjacent)', km: 5, note: 'nearest urban area; ~HK$75–100' },
    ],
    tips: [
      'Red urban taxis operate from HKG to Kowloon and Hong Kong Island — they are metered and fully regulated.',
      'Cross-harbour tunnel tolls are paid by the passenger on top of the metered fare; the driver pays the outbound toll.',
      'Airport Express train runs to Kowloon and Hong Kong stations in 20–24 minutes for HK$115 — best option for most travellers.',
      'Octopus card accepted on Airport Express and most buses — buy one at the airport.',
      'Late-night (00:00–06:00) surcharge applies to all taxi fares.',
    ],
    scams: [
      'Driver demands a flat rate instead of using the meter — red taxis are always metered.',
      '"Limousine" or "private car" touts at arrivals quoting flat rates double the taxi fare.',
      'Incorrect change given — always check before leaving the taxi.',
      'Driver claims the tunnel tolls are much higher than official rates — verify on the Transport Department website.',
    ],
    alternatives: [
      'Airport Express — HK$115 to Kowloon, HK$115 to Hong Kong Station (HK Island), 20–24 min',
      'Bus routes (E11, A21, A11) — HK$33–48, 50–70 min, good value with luggage space',
      'Ferry to various piers — seasonal, good for scenic approach to city',
    ],
  },
  ...
  YYZ: {
    ...
    ratePerKm: 2.25,
    minimumFare: 4.25,
    taxiColor: 'various (licensed metered)',
    approxCityFare: 'CA$55–80 to downtown Toronto',
    routes: [
      { label: 'Downtown Toronto / Union Station', km: 27, note: '20% tip customary; allow 30–60 min depending on traffic' },
      { label: 'Yorkville / Bloor', km: 25 },
      { label: 'Scarborough / East Toronto', km: 40 },
      { label: 'Mississauga City Centre', km: 14 },
      { label: 'Hamilton city centre', km: 55, note: 'negotiate fixed price; ~CA$80–100' },
    ],
    tips: [
      'All licenced taxis are metered — the standard rate is CA$4.25 flag fall plus CA$2.25/km.',
      'A 20% tip is customary and expected in Canada — factor this into the total fare.',
      'Uber and Lyft are widely available from Terminal 1 and 3 pickup zones — often cheaper than taxis.',
      'UP Express train runs to Union Station in 25 minutes for CA$12.35 — cheapest option to downtown.',
      'Traffic on the 427 and Gardiner Expressway can double journey times during peak hours (7–9 am, 4–7 pm).',
    ],
    scams: [
      'Driver quotes a flat rate above the metered equivalent — always insist on the meter.',
      'Unofficial limousine drivers approaching in arrivals before the taxi rank — use app-based or the official taxi line.',
      'Inflated flat rates to specific downtown hotels — the meter is always the legal standard.',
    ],
    alternatives: [
      'UP Express — CA$12.35 to Union Station, 25 min',
      'Uber / Lyft — pickup from designated rideshare zones, price shown upfront',
      'Bus 900 (Airport Rocket) — CA$3.35 to Kipling station (TTC subway), 20 min',
    ],
  },
  ...
  LIS: {
    ...
    ratePerKm: 0.50,
    minimumFare: 4.00,
    taxiColor: 'green and black (Táxi Lisboa)',
    approxCityFare: '€15–25 to Lisbon city centre',
    blogSlug: 'how-much-does-a-taxi-cost-in-lisbon',
    routes: [
      { label: 'Baixa / Chiado (city centre)', km: 8, note: 'closest major area; 15–20 min' },
      { label: 'Alfama / Castelo', km: 9 },
      { label: 'Belém / LX Factory', km: 13 },
      { label: 'Parque das Nações (Expo area)', km: 5, note: 'closest major district to airport' },
      { label: 'Sintra', km: 35, note: 'negotiate fixed price; ~€50–65; train from Rossio is far cheaper' },
      { label: 'Cascais', km: 35, note: 'negotiate fixed price; ~€50–65' },
    ],
    tips: [
      'Lisbon airport is just 8 km from the city centre — taxi fares are among the most affordable in Western Europe.',
      'The metered fare uses Tariff 1 (day, weekdays) or Tariff 2 (nights, weekends, and distances over 50 km).',
      'Bolt and Uber are widely available and price is fixed upfront — often matches or beats the metered taxi.',
      'Aerobus 1 and Metro (Red Line to Oriente, then Green/Yellow to centre) are both cheap alternatives.',
      'Night surcharge (21:00–06:00) and weekend rates add ~20% to the metered fare.',
    ],
    scams: [
      'Driver uses Tariff 2 (night rate) during daytime — check the tariff display on the meter.',
      'Unlicensed drivers outside the terminal claiming to be taxis — all licensed Lisbon taxis are green and black.',
      'Driver offers a flat rate to Sintra or Cascais that significantly exceeds the metered equivalent.',
    ],
    alternatives: [
      'Metro Red Line — €1.70 to Oriente (change for city centre), 10 min to Oriente',
      'Aerobus 1 — €4.50 to Cais do Sodré via Marquês de Pombal, 30–40 min',
      'Bolt / Uber — reliable and transparent pricing throughout Lisbon',
    ],
  },
  ...
  DUB: {
    ...
    ratePerKm: 1.14,
    minimumFare: 4.10,
    taxiColor: 'any colour (SPSV licence required)',
    approxCityFare: '€25–40 to Dublin city centre',
    routes: [
      { label: 'Dublin city centre / O\'Connell Street', km: 13, note: '20–35 min depending on traffic' },
      { label: 'Temple Bar / Trinity College', km: 14 },
      { label: 'Ballsbridge / Donnybrook', km: 16 },
      { label: 'Dún Laoghaire / Blackrock', km: 23, note: '~€35–55' },
      { label: 'Belfast city centre', km: 168, note: 'very expensive; bus or train strongly recommended' },
    ],
    tips: [
      'All licensed Irish taxis are metered and regulated by the National Transport Authority — the meter is mandatory.',
      'Dublin Airport taxi ranks are at arrivals level of Terminal 1 and Terminal 2.',
      'Free Now (formerly MyTaxi) and Uber both operate in Dublin with upfront pricing.',
      'Night surcharge (20:00–08:00) adds ~20% to all metered fares.',
      'Airlink Express buses (Routes 747 and 757) are cheaper and run directly to the city centre.',
    ],
    scams: [
      'Driver quotes a flat rate to the city centre — the meter is always required for journeys under 30 km.',
      'Route choice via the M50 motorway without mentioning the potential for toll costs — confirm route upfront.',
      'Unlicensed taxi touts inside the terminal — all legitimate taxis queue at the designated rank.',
    ],
    alternatives: [
      'Airlink Express 747 — €10 to city centre (stops at O\'Connell Street & Heuston), 30–45 min',
      'Airlink Express 757 — €10 to Connolly Station and George\'s Quay, 35–45 min',
      'Free Now / Uber — price shown upfront, widely used in Dublin',
    ],
  },
}
...
const AIRPORT_KEYWORDS: Record<string, string[]> = {
  ...
  MEL: ['tullamarine', 'melbourne airport'],
  IST: ['istanbul airport', 'yeni havalimani'],
  KUL: ['klia', 'kuala lumpur international'],
  BCN: ['el prat', 'barcelona airport'],
  MUC: ['munich airport', 'flughafen münchen'],
  HKG: ['hong kong international', 'chek lap kok'],
  YYZ: ['pearson', 'toronto pearson'],
  LIS: ['humberto delgado', 'portela', 'lisbon airport'],
  DUB: ['dublin airport', 'collinstown'],
}
